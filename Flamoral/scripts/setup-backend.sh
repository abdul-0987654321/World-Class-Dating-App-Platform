...
# Usage:
#   ./setup-backend.sh
#   ./setup-backend.sh --subscription "your-subscription-id" --location "eastus2"
#
...
RESOURCE_GROUP_NAME="${RESOURCE_GROUP_NAME:-rg-terraform-state-westus2}"
STORAGE_ACCOUNT_NAME="${STORAGE_ACCOUNT_NAME:-sttfstatedatingplatform}"
...
echo ""
echo -e "${YELLOW}Enabling soft delete for blobs (14 days retention)...${NC}"
az storage account blob-service-properties update \
    --account-name "${STORAGE_ACCOUNT_NAME}" \
    --resource-group "${RESOURCE_GROUP_NAME}" \
...
echo ""
echo -e "${YELLOW}Enabling soft delete for containers (14 days retention)...${NC}"
az storage account blob-service-properties update \
    --account-name "${STORAGE_ACCOUNT_NAME}" \
    --resource-group "${RESOURCE_GROUP_NAME}" \
...
echo ""
echo -e "${YELLOW}Use these values in your backend.tf:${NC}"
cat << EOF
terraform {
  backend "azurerm" {
...
echo "1. Run configure-sp.sh to set up service principal authentication"

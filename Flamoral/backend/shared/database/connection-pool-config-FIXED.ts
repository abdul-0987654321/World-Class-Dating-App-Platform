...
export function getPostgresConfig(
  ...
  return {
    ...
    statement_timeout: parseInt(process.env.DB_QUERY_TIMEOUT || (isProd ? '30000' : '60000'), 10),

    // Query timeout
    query_timeout: parseInt(process.env.DB_QUERY_TIMEOUT || (isProd ? '30000' : '60000'), 10),
    ...
    application_name: process.env.DB_APPLICATION_NAME || `flamoral_${environment}`,
...
export function getOptimizedKnexConfig(
  ...
  const isProd = environment === 'production';
  const postgresConfig = getPostgresConfig(environment);

  // Override database name if provided
  if (databaseName) {
    postgresConfig.database = databaseName;
  }

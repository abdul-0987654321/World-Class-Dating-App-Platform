    ...
    try {
      ...
      if (serviceAccountPath) {
        try {
          ...
          logger.warn(`Failed to load service account from file: ${err.message}. Trying environment variables...`);
        }
      }
      ...
      if (!privateKey || !clientEmail || !projectId) {
        logger.warn('Firebase not configured. Push notifications will be disabled. Set FIREBASE_SERVICE_ACCOUNT_PATH, FIREBASE_SERVICE_ACCOUNT, or individual environment variables (FIREBASE_PROJECT_ID, FIREBASE_CLIENT_EMAIL, FIREBASE_PRIVATE_KEY).');

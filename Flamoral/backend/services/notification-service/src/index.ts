...
import { validateConfiguration, logValidationResults, printConfigurationSummary } from './config/validation';

const validationResult = validateConfiguration();
logValidationResults(validationResult);

if (!validationResult.valid) {
  logger.error('Configuration validation failed. Service cannot start.');
  process.exit(1);
}

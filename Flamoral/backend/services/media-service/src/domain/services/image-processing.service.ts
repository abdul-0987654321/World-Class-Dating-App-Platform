...
export class ImageProcessingService {
  ...
  async createThumbnail(buffer: Buffer): Promise<Buffer> {
    try {
      ...
      // Use WebP if enabled for better compression
      if (config.imageProcessing.enableWebP) {
        return await pipeline
          .webp({ quality: config.imageProcessing.webpQuality })
          .toBuffer();
      }

      return await pipeline
        .jpeg({ quality: config.imageProcessing.jpegQuality })
        .toBuffer();
    } catch (error) {

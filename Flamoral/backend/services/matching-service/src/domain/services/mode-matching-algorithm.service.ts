...
export class ModeMatchingAlgorithmService {
  ...
  private calculateDistance(
    ...
    if (Math.abs(loc1.latitude) > 90 || Math.abs(loc2.latitude) > 90 ||
      ...
      return Infinity; // Return max distance if out of range
    }
  ...
  private calculateInterestsScore(
    ...
    const safeInterests1 = Array.isArray(interests1) ? interests1 : [];
    const safeInterests2 = Array.isArray(interests2) ? interests2 : [];

    if (safeInterests1.length === 0 || safeInterests2.length === 0) {
      return 0.5; // Neutral score if no interests
    }

    const common = safeInterests1.filter((i) =>
      safeInterests2.some((j) => j.toLowerCase() === i.toLowerCase())
    );

    const totalUnique = new Set([...safeInterests1, ...safeInterests2]).size;

    // Use Jaccard similarity: intersection / union
    const jaccardSimilarity = totalUnique > 0 ? common.length / totalUnique : 0.5;

    // In friends mode, reward exact matches more heavily
    if (strictMatch) {
      return Math.min(jaccardSimilarity * 1.5, 1.0);
    }
  ...
  private calculateProfessionalInterestsScore(
    ...
    const overlapScore = this.calculateInterestsScore(safeInterests1, safeInterests2);

    // Bonus for diversity (complementary skills)
    const uniqueCount = new Set([...safeInterests1, ...safeInterests2]).size;
    const totalCount = safeInterests1.length + safeInterests2.length;
    const diversityBonus = totalCount > 0 ? (uniqueCount / totalCount) * 0.3 : 0;
  ...
  private calculateWeightedScore(
    ...
    for (const key in factors) {
      if (weights[key] !== undefined && typeof factors[key] === 'number' && !isNaN(factors[key])) {
        // Ensure factor is between 0 and 1
        const normalizedFactor = Math.max(0, Math.min(1, factors[key]));
        totalScore += normalizedFactor * weights[key];
...
export default new ModeMatchingAlgorithmService();

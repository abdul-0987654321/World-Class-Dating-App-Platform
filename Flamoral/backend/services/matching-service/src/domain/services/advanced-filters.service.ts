...
export class AdvancedFiltersService {
  ...
  buildFilterQuery(filters: AdvancedFilters): {
    ...
    if (!validation.valid) {
      logger.warn('Invalid filters provided', { errors: validation.errors });
      return { conditions: [], parameters: [] };
    }

    // Height filters
    if (filters.heightMin !== undefined && typeof filters.heightMin === 'number') {
      conditions.push(`height >= $${paramIndex}`);
      parameters.push(filters.heightMin);
      paramIndex++;
    }

    if (filters.heightMax !== undefined && typeof filters.heightMax === 'number') {
      conditions.push(`height <= $${paramIndex}`);
    ...
    // Occupation filter
    if (Array.isArray(filters.occupation) && filters.occupation.length > 0) {
      conditions.push(`occupation = ANY($${paramIndex})`);
      parameters.push(filters.occupation);
      paramIndex++;
    }
    ...
    if (Array.isArray(filters.ethnicity) && filters.ethnicity.length > 0) {
      ...
      parameters.push(filters.ethnicity);
      paramIndex++;
    }

    // Religion filter
    if (Array.isArray(filters.religion) && filters.religion.length > 0) {
    ...
    // Politics filter
    if (Array.isArray(filters.politics) && filters.politics.length > 0) {
    ...
    // Lifestyle filters
    if (Array.isArray(filters.smoking) && filters.smoking.length > 0) {
      conditions.push(`smoking = ANY($${paramIndex})`);
      parameters.push(filters.smoking);
      paramIndex++;
    }

    if (Array.isArray(filters.drinking) && filters.drinking.length > 0) {
      conditions.push(`drinking = ANY($${paramIndex})`);
      parameters.push(filters.drinking);
      paramIndex++;
    }

    if (Array.isArray(filters.exercise) && filters.exercise.length > 0) {
      conditions.push(`exercise = ANY($${paramIndex})`);
      parameters.push(filters.exercise);
      paramIndex++;
    }

    if (Array.isArray(filters.diet) && filters.diet.length > 0) {
    ...
    // Relationship preferences
    if (Array.isArray(filters.relationshipType) && filters.relationshipType.length > 0) {
      conditions.push(`relationship_type = ANY($${paramIndex})`);
      parameters.push(filters.relationshipType);
      paramIndex++;
    }

    if (Array.isArray(filters.lookingFor) && filters.lookingFor.length > 0) {
      conditions.push(`looking_for && $${paramIndex}`);
      parameters.push(filters.lookingFor);
      paramIndex++;
    }
    ...
    if (Array.isArray(filters.petPreference) && filters.petPreference.length > 0) {
      conditions.push(`pet_type = ANY($${paramIndex})`);
      parameters.push(filters.petPreference);
      paramIndex++;
    }
    ...
    // Photos minimum
    if (filters.hasPhotosMin !== undefined && typeof filters.hasPhotosMin === 'number') {
    ...
    // Zodiac signs
    if (Array.isArray(filters.zodiacSigns) && filters.zodiacSigns.length > 0) {
    ...
    // Personality type
    if (Array.isArray(filters.personalityType) && filters.personalityType.length > 0) {
      conditions.push(`personality_type = ANY($${paramIndex})`);
      parameters.push(filters.personalityType);
      paramIndex++;
    }

    // Love language
    if (Array.isArray(filters.loveLanguage) && filters.loveLanguage.length > 0) {
      conditions.push(`love_language = ANY($${paramIndex})`);
      parameters.push(filters.loveLanguage);
      paramIndex++;
    }

    // Distance override
    if (filters.maxDistance !== undefined && typeof filters.maxDistance === 'number') {
      // This would need special handling with geospatial queries
      // For now, we just note it should be handled separately
      logger.debug(`Max distance filter: ${filters.maxDistance}km`);
    }

    // Minimum compatibility score
    if (filters.minCompatibilityScore !== undefined && typeof filters.minCompatibilityScore === 'number') {
      conditions.push(`compatibility_score >= $${paramIndex}`);
      parameters.push(filters.minCompatibilityScore);
      paramIndex++;
    }

    // Exclude previously passed
    if (filters.excludePreviouslyPassed === true) {
      // This would need to join with swipes table
      logger.debug('Excluding previously passed users');
    }

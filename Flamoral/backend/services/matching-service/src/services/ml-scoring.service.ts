...
class MLScoringService {
  ...
  private calculateInterestAlignment(features: any): number {
    ...
    // Handle null/undefined interests
    const userInterestsArray = Array.isArray(user.interests) ? user.interests : [];
    const targetInterestsArray = Array.isArray(target.interests) ? target.interests : [];

    const userInterests = new Set(userInterestsArray.map((i: string) => i.toLowerCase()));
    const targetInterests = new Set(targetInterestsArray.map((i: string) => i.toLowerCase()));

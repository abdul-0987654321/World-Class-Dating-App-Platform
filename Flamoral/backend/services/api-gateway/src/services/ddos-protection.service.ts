...
export class DDoSProtectionService {
  ...
  constructor(private readonly configService: ConfigService) {
    // Initialize Redis connection with TLS support for Azure Redis and production-ready settings
    this.redis = new Redis({
      ...
      retryStrategy: (times) => {
        if (times > 10) {
          this.logger.error('Redis connection failed after 10 retries, disabling DDoS protection');
    ...
    this.redis.on('ready', () => {
      this.redisHealthy = true;
      this.redisFailureCount = 0;
      this.logger.log('Redis ready for DDoS protection');
    });
  ...
  async checkRequest(req: Request, clientIp: string): Promise<DDoSCheckResult> {
    try {
      ...
      if (!this.redisHealthy) {
        this.logger.warn('Redis unhealthy, DDoS protection disabled (fail-open)');
        return { allowed: true };
      }
      ...
      this.logger.error('DDoS check error (failing open):', error);
      this.redisFailureCount++;
      if (this.redisFailureCount >= this.redisFailureThreshold) {
        this.redisHealthy = false;
      }
  ...
  private async incrementCounter(
    ...
    // FAIL OPEN: If Redis is unhealthy, return 0 (no limit enforcement)
    if (!this.redisHealthy) {
      return 0;
    }
    ...
    try {
      ...
      this.logger.error('Increment counter failed (failing open):', error);
      this.redisFailureCount++;
      if (this.redisFailureCount >= this.redisFailureThreshold) {
        this.redisHealthy = false;
      }

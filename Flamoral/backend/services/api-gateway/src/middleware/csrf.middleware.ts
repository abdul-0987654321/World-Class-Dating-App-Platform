...
export class CsrfMiddleware implements NestMiddleware {
  ...
  private readonly excludedPaths = [
    ...
    '/api/health',
    '/api/metrics',
    // Auth endpoints - public endpoints that don't need CSRF
    '/api/auth/register',
    '/api/auth/login',
    ...
    // Safety endpoints - public resources
    '/api/safety/tips',
    '/api/safety/crisis',
    '/api/safety/resources',
    // Legacy paths (with /api/v1/api prefix) - for backwards compatibility
    '/api/v1/api/webhooks',
    '/api/v1/api/health',
    '/api/v1/api/auth/register',
    '/api/v1/api/auth/login',
    '/api/v1/api/auth/forgot-password',
    '/api/v1/api/auth/reset-password',
    '/api/v1/api/auth/verify-email',
    '/api/v1/api/auth/resend-verification',
    '/api/v1/api/auth/refresh-token',
    '/api/v1/api/auth/oauth/google',
    '/api/v1/api/auth/oauth/facebook',
    '/api/v1/api/auth/oauth/apple',
    // Safety endpoints - public resources (with prefix)
    '/api/v1/api/safety/tips',
    '/api/v1/api/safety/crisis',
    '/api/v1/api/safety/resources',
  ];
  ...
  private generateAndSetToken(req: Request, res: Response): void {
    ...
    // Determine cookie settings based on environment and cross-origin needs
    const isProduction = this.configService.get('NODE_ENV') === 'production';
    const origin = req.headers.origin || req.headers.referer;
    const isCrossOrigin = origin && !origin.includes(req.headers.host || '');

    // For cross-origin requests, use SameSite=None with Secure flag
    // For same-origin, use SameSite=Lax for better compatibility while maintaining security
    const cookieOptions = {
      secure: isProduction, // Always secure in production
      sameSite: (isCrossOrigin && isProduction) ? 'none' as const : 'lax' as const,
      maxAge: this.tokenExpiry,
      path: '/',
      domain: isProduction ? '.flamoral.com' : undefined, // Allow cross-subdomain in production
    };

 ...
 * - Add CDN domain to CSP directives
...
export class SecurityHeadersMiddleware implements NestMiddleware {
  ...
  constructor(private readonly configService: ConfigService) {
    ...
    this.webDomain = this.configService.get<string>('WEB_DOMAIN') || 'https://flamoral.com';
    this.cdnDomain = this.configService.get<string>('CDN_DOMAIN') || 'https://cdn.flamoral.com';
  }
  ...
  use(req: Request, res: Response, next: NextFunction) {
    ...
    // max-age=63072000: 2 years (recommended for production)
  ...
  private buildCSPDirectives(): string {
    const directives = {
      ...
      'script-src': this.isDevelopment
        ? ["'self'", "'unsafe-inline'", "'unsafe-eval'", 'https://js.stripe.com', 'https://www.google-analytics.com', 'https://www.googletagmanager.com', 'https://www.google.com', 'https://www.gstatic.com'] // Allow inline scripts in dev for HMR + reCAPTCHA
        : ["'self'", 'https://js.stripe.com', 'https://www.google-analytics.com', 'https://www.googletagmanager.com', 'https://www.google.com', 'https://www.gstatic.com'], // Strict in production + reCAPTCHA
      'style-src': ["'self'", "'unsafe-inline'", 'https://fonts.googleapis.com'], // unsafe-inline needed for styled-components
      'img-src': ["'self'", 'data:', 'https:', 'blob:', this.cdnDomain],
      'font-src': ["'self'", 'data:', 'https://fonts.gstatic.com', this.cdnDomain],
      'connect-src': [
        "'self'",
        'wss://*.flamoral.com',
        'ws://*.flamoral.com',
        ...
        ...(this.isDevelopment ? ['http://localhost:*', 'ws://localhost:*', 'wss://localhost:*'] : [])
      ],
      'media-src': ["'self'", 'blob:', 'data:', 'https:', this.cdnDomain],
      'object-src': ["'none'"], // Disable plugins
      'frame-src': ["'self'", 'https://js.stripe.com', 'https://www.google.com', 'https://www.gstatic.com'], // Allow Stripe iframes and reCAPTCHA
      'frame-ancestors': this.isDevelopment ? ["'self'", 'http://localhost:*'] : ["'none'"], // Prevents embedding except in development
      'base-uri': ["'self'"], // Restricts <base> tag URLs
      'form-action': ["'self'", 'https://*.flamoral.com'], // Allow form submission to flamoral domains
      'manifest-src': ["'self'"],

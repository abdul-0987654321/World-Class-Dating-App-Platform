...
/**
 * @swagger
 * /api/video-chat/entitlements:
 *   get:
 *     summary: Get user's video call entitlements
 *     tags: [VideoChat]
 *     security:
 *       - bearerAuth: []
 *     description: |
 *       Returns user's video call entitlements based on subscription tier.
 *       Used by frontend to show/hide call buttons and enforce limits.
 *     responses:
 *       200:
 ...
 *                 tierRequired:
 *                   type: string
 *                   description: Minimum tier for video calls
 */
router.get('/entitlements', requireAuth, async (req: Request, res: Response) => {
  try {
    ...
    const tierEntitlements: Record<string, {
    ...
    // Calculate remaining calls today if there's a limit
    let remainingCallsToday = entitlements.callsPerDay;

    if (entitlements.callsPerDay > 0) {
      ...
      const callCount = Number(callsToday?.count || 0);
      remainingCallsToday = Math.max(0, entitlements.callsPerDay - callCount);
    }

    res.status(200).json({
      canMakeVideoCalls: entitlements.canMakeVideoCalls,
      canMakeAudioCalls: entitlements.canMakeAudioCalls,
      maxMinutesPerCall: entitlements.maxMinutesPerCall,
      callsPerDay: entitlements.callsPerDay,
      remainingCallsToday,
      tierRequired: entitlements.canMakeVideoCalls ? undefined : 'BASIC',
    });
  } catch (error: any) {
    logger.error('Failed to get video call entitlements', {
      userId: req.user?.id,
      error: error.message,
    });

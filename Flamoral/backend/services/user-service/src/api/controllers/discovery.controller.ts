...
export class DiscoveryController {
  ...
  async getDiscoveryStats(req: AuthRequest, res: Response): Promise<Response> {
    try {
      const userId = req.user!.userId;

      const stats = await this.discoveryService.getDiscoveryStats(userId);
      ...
      return res.status(500).json({
        success: false,
        message: error.message || 'Failed to retrieve discovery stats',
      });
    }
  }
}

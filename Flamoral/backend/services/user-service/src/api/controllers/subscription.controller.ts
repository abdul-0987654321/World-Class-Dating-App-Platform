...
export class SubscriptionController {
  ...
  async getCurrentSubscription(req: AuthRequest, res: Response): Promise<Response> {
    try {
      ...
      const statusDetails = await this.subscriptionService.getSubscriptionStatusDetails(userId);

      // Transform features into a simple list of feature keys
      const featuresList = features
        .filter(f => f.active && f.featureValue?.enabled !== false)
        .map(f => f.featureKey);
      ...
      return res.status(200).json({
        ...
        data: {
          ...
          isInTrial: statusDetails.isInTrial,
          isInGracePeriod: statusDetails.isInGracePeriod,
  ...
  async updateTier(req: AuthRequest, res: Response): Promise<Response> {
    try {
      ...
      if (!tier || !['free', 'basic', 'plus', 'premium', 'premium_plus', 'elite'].includes(tier)) {

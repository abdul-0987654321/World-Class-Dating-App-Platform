  ...
  async submitPhoto(verificationId: string, photoUrl: string): Promise<PhotoVerificationResult> {
    try {
      ...
      if (result.verified) {
        ...
        // Award photo verification badge
        await verificationBadgeService.onPhotoVerified(request.user_id);
  ...
  async getVerificationStatus(userId: string): Promise<any> {
    try {
      const user = await db('users')
        .where({ id: userId })
        .first('is_photo_verified', 'photo_verified_at');
      ...
      return {
        verified: user?.is_photo_verified || false,

...
export class CoinService {
   ...
   * Auto-initializes coin account with welcome bonus if not exists
   */
  async getBalance(userId: string): Promise<Coin> {
    let coin = await this.coinRepository.findByUserId(userId);

    // Auto-initialize coin account if it doesn't exist
    if (!coin) {
      const WELCOME_BONUS_COINS = 50; // Welcome bonus for new users
      coin = await this.initializeCoinAccount(userId, WELCOME_BONUS_COINS);
    }
  ...
  /**
   * Get user's coin balance without auto-initialization
   * Used for admin/internal purposes
   */
  async getBalanceIfExists(userId: string): Promise<Coin | null> {
  ...
  async spendCoins(
    ...
    // Get or create coin account (ensures all users have an account)
    const currentCoin = await this.getBalance(userId);
  ...
  async awardCoins(
    ...
    // Ensure coin account exists (auto-initialize if not)

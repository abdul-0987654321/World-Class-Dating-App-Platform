...
// createUpgrader creates a WebSocket upgrader with proper CORS validation
func (s *Server) createUpgrader() websocket.Upgrader {
	return websocket.Upgrader{
		...
		CheckOrigin: func(r *http.Request) bool {
			...
			// Validate against allowed origins from config
			...
			log.WithField("origin", origin).Warn("WebSocket connection rejected: origin not allowed")
			return false
		},
	}
}

// Default upgrader for backward compatibility (will be replaced by createUpgrader)
var upgrader = websocket.Upgrader{
	ReadBufferSize:  1024,
	WriteBufferSize: 1024,
	CheckOrigin: func(r *http.Request) bool {
		// Default: reject all unless proper upgrader is used
		// This ensures security if createUpgrader is not called
		return false
	},
}
...
func (s *Server) wsHandler(w http.ResponseWriter, r *http.Request) {
	...
	// Use secure upgrader with proper CORS validation
	secureUpgrader := s.createUpgrader()
	secureUpgrader.HandshakeTimeout = s.config.WSHandshakeTimeout

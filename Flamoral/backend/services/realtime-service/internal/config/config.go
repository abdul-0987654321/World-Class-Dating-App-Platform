...
type Config struct {
	...
	RedisDB       int
	RedisTLS      bool // ADDED: TLS support for Azure Redis
...
func Load() *Config {
	...
	cfg := &Config{
		...
		RedisTLS:      getEnvBool("REDIS_TLS", false), // ADDED

		// JWT
		JWTSecret:     getEnvRequired("JWT_SECRET"),  // No default - must be configured
		JWTIssuer:     getEnv("JWT_ISSUER", "flamoral-auth-service"),
		JWTExpiration: getEnvDuration("JWT_EXPIRATION", 15*time.Minute),

		// Service auth
		ServiceToken: getEnvRequired("SERVICE_TOKEN"), // No default - must be configured

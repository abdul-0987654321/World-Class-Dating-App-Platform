...
export class SocketManager {
  ...
  private handleConnection(socket: Socket): void {
    ...
    // Get userId from handshake (support multiple auth methods)
    const userId = socket.handshake.auth?.userId ||
                   socket.handshake.auth?.user?.userId ||
                   socket.handshake.query?.userId as string;

    if (!userId) {
      logger.warn(`Connection rejected: No userId provided for socket ${socket.id}`);
      socket.emit('error', {
        message: 'Authentication required. Please provide userId in auth or query params.',
        code: 'AUTH_REQUIRED'
      });
    ...
    if (typeof userId !== 'string' || userId.length < 1) {
      logger.warn(`Connection rejected: Invalid userId format for socket ${socket.id}`);
      socket.emit('error', {
  ...
  private registerDisconnectHandler(socket: Socket, userId: string): void {
    ...
    socket.on('reconnect_attempt', () => {
      logger.debug(`User ${userId} attempting to reconnect`);
    });
  }

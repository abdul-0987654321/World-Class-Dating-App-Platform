...
import { messageEncryptionHandler } from '../../domain/services/message-encryption-handler.service';
...
export class MessageController {
  ...
  async getMessages(req: AuthRequest, res: Response): Promise<Response> {
    try {
      ...
      // Process messages through encryption handler (returns messages as-is for client-side decryption)
      const processedMessages = await Promise.all(
        filteredMessages.map(msg => messageEncryptionHandler.processOutgoingMessage(msg))
      );
  ...
  async sendMessage(req: AuthRequest, res: Response): Promise<Response> {
    try {
      const userId = req.user!.userId;
      const { conversationId, receiverId, content, type, metadata, replyTo, encryption } = req.body;
      ...
      // Process encryption if provided
      let processedMessage;
      try {
        processedMessage = await messageEncryptionHandler.processIncomingMessage({
          content,
          encryption,
        });
      } catch (encryptionError: any) {
        logger.error('Encryption processing error:', encryptionError);
        return res.status(400).json({
          success: false,
          error: `Message encryption failed: ${encryptionError.message}`,
        });
      }
      ...
      const message: Message = {
        ...
        senderId: userId,
        receiverId,
        content: processedMessage.content, // Use processed (potentially encrypted) content
        type: type || MessageType.TEXT,
        status: MessageStatus.SENT,
        sentAt: new Date(),
        metadata,
        replyTo,
        encryption: processedMessage.encryption, // Store encryption metadata
      };

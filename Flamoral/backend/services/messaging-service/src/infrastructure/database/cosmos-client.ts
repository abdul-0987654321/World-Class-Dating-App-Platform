  ...
  async initialize(): Promise<void> {
    ...
    try {
      ...
      const { container: callHistoryContainer } = await database.containers.createIfNotExists({
        id: config.cosmos.containers.callHistory,
      ...
      logger.info(`Container "${config.cosmos.containers.callHistory}" ready`);
      ...
      const { container: callRecordingsContainer } = await database.containers.createIfNotExists({
        id: config.cosmos.containers.callRecordings,
        partitionKey: '/callId',
        indexingPolicy: {
          automatic: true,
          indexingMode: 'consistent',
      ...
      logger.info(`Container "${config.cosmos.containers.callRecordings}" ready`);

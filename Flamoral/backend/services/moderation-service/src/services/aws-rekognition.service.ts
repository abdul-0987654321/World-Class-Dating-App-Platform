  ...
  constructor() {
    ...
    if (config.aws.accessKeyId && config.aws.secretAccessKey) {
      try {
        ...
        logger.warn('Image moderation will use fallback mechanisms');
      }
    } else {
      logger.warn('AWS credentials not configured. Image moderation will be limited to basic checks.');
    }
  }
  ...
  async moderateImage(imageUrl: string): Promise<ImageModerationResult> {
    try {
      ...
      if (!this.isConfigured()) {
        logger.warn('AWS Rekognition not configured, returning safe default result');
        return this.createSafeDefaultResult();
      }
      ...
      logger.warn('Returning safe default result due to error');
      // Return safe default that flags for manual review instead of failing
      return this.createSafeDefaultResult();
    }
  }

  /**
   * Create a safe default result when service is unavailable
   */
  private createSafeDefaultResult(): ImageModerationResult {
    return {
      moderationLabels: [],
      categories: {
        explicitNudity: 0,
      ...
      overallRiskScore: 0.5, // Medium risk to trigger manual review
      detectedViolations: [],
      recommendations: ['Manual review recommended - automated moderation unavailable'],
    };
  }

...
class EmailService {
  ...
  private async sendViaProvider(options: {
    ...
    const from = `"Flamoral" <${config.email.from}>`;
  ...
  async sendVerificationEmail(email: string, name: string, token: string): Promise<void> {
    ...
    try {
      ...
      logger.error('Failed to send verification email', { error, provider: this.provider });
  ...
  async sendPasswordResetEmail(email: string, name: string, token: string): Promise<void> {
    ...
    try {
      ...
      logger.error('Failed to send password reset email', { error, provider: this.provider });
  ...
  async sendWelcomeEmail(email: string, name: string): Promise<void> {
                  ...
                  <tr>
                    <td style="padding: 40px 30px;">
                      ...
                      <table width="100%" cellpadding="0" cellspacing="0" border="0" style="margin: 30px 0;">
                        <tr>
                          <td align="center">
                            <a href="${config.frontendUrl}/discover" style="display: inline-block; background-color: #E91E63; color: #ffffff; padding: 15px 40px; text-decoration: none; border-radius: 5px; font-weight: bold; font-size: 16px;">Start Discovering</a>
                          </td>
                        </tr>
                      </table>
                    </td>
    ...
    const text = `
      ...
      Visit ${config.frontendUrl}/discover to start discovering matches!
    ...
    try {
      ...
      logger.error('Failed to send welcome email', { error, provider: this.provider });

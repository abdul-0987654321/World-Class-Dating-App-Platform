  ...
  async refreshToken(refreshToken: string): Promise<TokenPair> {
    try {
      ...
      if (storedToken !== refreshToken) {
        // Token doesn't match - check if it was already used (token reuse attack)
        if (payload.jti) {
        ...
        // Token doesn't match and wasn't marked as used - possible token theft
        await redisCache.invalidateAllUserTokens(payload.userId);
      ...
      // Mark old refresh token as used before removing it
      if (payload.jti) {
        await redisCache.set(`refresh_token_used:${payload.jti}`, '1', 7 * 24 * 60 * 60); // 7 days

...
export class NotificationServiceClient {
  ...
  constructor() {
    this.baseUrl = process.env.NOTIFICATION_SERVICE_URL || 'http://localhost:3012';
    this.timeout = parseInt(process.env.NOTIFICATION_SERVICE_TIMEOUT || '5000', 10);
    ...
    logger.info(`NotificationServiceClient initialized with baseUrl: ${this.baseUrl}`);
  }
  ...
  async sendNotification(data: SendNotificationDto): Promise<void> {
    try {
      ...
      logger.info(`Successfully sent notification to user ${data.userId}`);
    } catch (error: any) {
      // Don't throw - notifications are non-critical
      logger.warn(`Failed to send notification to user ${data.userId}:`, {
        error: error.message,
        type: data.type,
        title: data.title,
      });

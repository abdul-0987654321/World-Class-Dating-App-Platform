...
async def lifespan(app: FastAPI) -> AsyncGenerator:
    ...
    logger.info("Starting recommendation service", version=settings.VERSION)
...
async def readiness_check():
    ...
    try:
        is_ready = await app.state.vector_store.is_healthy()
        if not is_ready:
            return {"status": "not ready", "error": "Vector store not available"}
    except Exception as e:

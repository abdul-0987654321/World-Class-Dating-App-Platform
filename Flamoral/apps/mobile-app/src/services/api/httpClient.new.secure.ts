/**
 * HTTP Client for Flamoral Mobile App - SECURE VERSION
 * Provides standardized HTTP methods with error handling, retries, and secure auth
 * Uses secure storage (Keychain/Keystore) instead of AsyncStorage
 */
  ...
  private async request<T>(
    ...
    while (retryCount <= maxRetries) {
      try {
        ...
        if (response.status === 401 && !options.skipAuth && !endpoint.includes('/refresh-token')) {

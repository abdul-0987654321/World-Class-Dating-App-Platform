...
export const CoinShopPage: React.FC = () => {
  ...
  const loadData = async () => {
    try {
      ...
      // Load packages
      const pkgRes = await fetch('/api/v1/coins/packages', { credentials: 'include', headers });
      if (pkgRes.ok) {
      ...
      // Load balance
      const balRes = await fetch('/api/v1/coins/balance', { credentials: 'include', headers });
      if (balRes.ok) {
  ...
  const handlePurchase = async (pkg: CoinPackage) => {
    ...
    try {
      const res = await fetch('/api/v1/coins/purchase', {
        method: 'POST',
        credentials: 'include',
        headers: {

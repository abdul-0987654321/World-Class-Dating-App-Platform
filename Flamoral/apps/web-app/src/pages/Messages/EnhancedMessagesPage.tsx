...
export const EnhancedMessagesPage: React.FC = () => {
  ...
  const handleSendMedia = async (file: File, type: 'image' | 'video') => {
    ...
    try {
      ...
      const response = await fetch('/api/v1/messaging/media', {
        method: 'POST',
        credentials: 'include',
        body: formData,
      });
  ...
  const handleSendVoiceNote = async (audioBlob: Blob, duration: number) => {
    ...
    try {
      const response = await fetch('/api/v1/messaging/voice', {
        method: 'POST',
        credentials: 'include',
        body: formData,
      });
  ...
  const handleSendGift = async (gift: VirtualGift) => {
    ...
    try {
      const response = await fetch('/api/v1/messaging/gifts', {
        method: 'POST',
        credentials: 'include',
        body: JSON.stringify({
  ...
  const handleReaction = async (messageId: string, reaction: Reaction) => {
    try {
      await fetch(`/api/v1/messaging/messages/${messageId}/reactions`, {
        method: 'POST',
        credentials: 'include',

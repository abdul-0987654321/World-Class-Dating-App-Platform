...
class CoinService {
  // Only enable mock mode when explicitly enabled via VITE_USE_MOCKS
  private isMock = import.meta.env.VITE_USE_MOCKS === 'true';
  ...
  async getTransactionHistory(cursor?: string): Promise<{ transactions: CoinTransaction[]; nextCursor?: string }> {
    ...
    const url = cursor
      ? `/api/v1/coins/transactions?cursor=${cursor}`

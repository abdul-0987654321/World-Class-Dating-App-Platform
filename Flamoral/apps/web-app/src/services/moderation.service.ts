...
class ModerationService {
  ...
  async getViolationHistory(cursor?: string): Promise<ViolationHistoryResponse> {
    ...
    const url = cursor
      ? `/api/v1/moderation/violations?cursor=${cursor}`
      : '/api/v1/moderation/violations';
  ...
  async getViolationDetails(violationId: string): Promise<Violation> {
    ...
    const response = await fetch(`/api/v1/moderation/violations/${violationId}`, {
      credentials: 'include',
      headers: {
        'Content-Type': 'application/json',
      },
  ...
  async getAppealStatus(appealId: string): Promise<{
    ...
    const response = await fetch(`/api/v1/moderation/appeals/${appealId}`, {
      credentials: 'include',
      headers: {
        'Content-Type': 'application/json',
      },
  ...
  async getModerationQueue(filter?: { status?: string; priority?: string }): Promise<ModerationQueueItem[]> {
    ...
    const url = queryString ? `/api/v1/admin/moderation/queue?${queryString}` : '/api/v1/admin/moderation/queue';
  ...
  async reviewContent(params: {
    ...
    const response = await fetch(`/api/v1/admin/moderation/review/${params.moderationLogId}`, {
      method: 'POST',
      credentials: 'include',
      headers: {
        'Content-Type': 'application/json',
  ...
  async getStatistics(startDate?: string, endDate?: string): Promise<ModerationStatistics> {
    ...
    const queryString = params.toString();
    const url = queryString ? `/api/v1/admin/moderation/statistics?${queryString}` : '/api/v1/admin/moderation/statistics';
  ...
  async getUserStatus(userId: string): Promise<ModerationStatus> {
    ...
    const response = await fetch(`/api/v1/moderation/users/${userId}/status`, {
      credentials: 'include',
      headers: {
        'Content-Type': 'application/json',
      },

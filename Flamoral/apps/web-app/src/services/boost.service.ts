...
class BoostService {
  ...
  async getActiveBoost(): Promise<Boost | null> {
    ...
    const response = await fetch('/api/v1/boosts/active', {
      credentials: 'include',
      headers: {
  ...
  async getBoostHistory(): Promise<{ boosts: Boost[]; totalCount: number }> {
    ...
    const response = await fetch('/api/v1/boosts/history', {
      credentials: 'include',
      headers: {
        'Content-Type': 'application/json',
      },
  ...
  async getBoostPackages(): Promise<BoostPackage[]> {
    ...
    const response = await fetch('/api/v1/boosts/packages', {
      credentials: 'include',
      headers: {
        'Content-Type': 'application/json',
      },
  ...
  async activateBoost(packageId: string, paymentMethod: 'card' | 'coins' = 'coins'): Promise<ActivateBoostResponse> {
    ...
    const response = await fetch('/api/v1/boosts/activate', {
      method: 'POST',
      credentials: 'include',
      headers: {
        'Content-Type': 'application/json',
  ...
  async getBoostStats(): Promise<BoostStats> {
    ...
    const response = await fetch('/api/v1/boosts/stats', {
      credentials: 'include',
      headers: {
        'Content-Type': 'application/json',
      },

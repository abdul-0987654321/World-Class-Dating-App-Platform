...
class ProfileService {
  ...
  async updateProfile(data: UpdateProfileData): Promise<UserProfile> {
    ...
    return apiClient.patch<UserProfile>('/api/v1/profiles/me', data);
  }
  ...
  async updateSettings(data: UpdateSettingsData): Promise<ProfileSettings> {
    ...
    return apiClient.patch<ProfileSettings>('/api/v1/profiles/me/settings', data);
  }
  ...
  async uploadPhoto(file: File, isPrimary: boolean = false): Promise<ProfilePhoto> {
    ...
    const response = await fetch('/api/v1/profiles/me/photos', {
      method: 'POST',
  ...
  async deletePhoto(photoId: string): Promise<void> {
    ...
    await apiClient.delete(`/api/v1/profiles/me/photos/${photoId}`);
  }
  ...
  async reorderPhotos(photoIds: string[]): Promise<void> {
    ...
    await apiClient.put('/api/v1/profiles/me/photos/reorder', { photoIds });
  }
  ...
  async addPrompt(question: string, answer: string): Promise<ProfilePrompt> {
    ...
    return apiClient.post<ProfilePrompt>('/api/v1/profiles/me/prompts', {
      question,
      answer,
    });
  }
  ...
  async updatePrompt(promptId: string, answer: string): Promise<ProfilePrompt> {
    ...
    return apiClient.patch<ProfilePrompt>(`/api/v1/profiles/me/prompts/${promptId}`, {
      answer,
    });
  }
  ...
  async deletePrompt(promptId: string): Promise<void> {
    ...
    await apiClient.delete(`/api/v1/profiles/me/prompts/${promptId}`);
  }
  ...
  async requestVerification(): Promise<{ verificationUrl: string }> {
    ...
    return apiClient.post('/api/v1/profiles/me/verify');
  }

...
class MatchingService {
  ...
  // Only enable mock mode in development when explicitly enabled via VITE_USE_MOCKS=true
  ...
  async getMatches(cursor?: string): Promise<MatchesResponse> {
    ...
    const url = cursor
      ? `/api/v1/matching/matches?cursor=${cursor}`
  ...
  async getLikes(cursor?: string): Promise<LikesResponse> {
    ...
    const url = cursor
      ? `/api/v1/matching/likes?cursor=${cursor}`
  ...
  async unmatch(matchId: string): Promise<void> {
    ...
    await apiClient.delete(`/api/v1/matching/matches/${matchId}`);
  }
  ...
  async reportMatch(matchId: string, reason: string): Promise<void> {
    ...
    await apiClient.post(`/api/v1/matching/matches/${matchId}/report`, { reason });
  }
  ...
  async blockUser(userId: string): Promise<void> {
    ...
    await apiClient.post('/api/v1/users/block', { userId });
  }
  ...
  async getBlockedUsers(): Promise<{ users: MatchedUser[]; totalCount: number }> {
    ...
    return apiClient.get('/api/v1/users/blocked');
  }
}

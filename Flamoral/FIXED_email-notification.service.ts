...
export class EmailNotificationService {
  ...
  constructor() {
    // Use fallback values for configuration
    this.fromEmail = config.sendgrid.fromEmail || 'noreply@flamoral.com';
    this.fromName = config.sendgrid.fromName || 'Flamoral';
    this.baseUrl = config.webAppUrl || 'http://localhost:5173';

    try {
      // Determine email provider with proper validation
      // Check for valid SendGrid API key (not placeholder)
      if (config.sendgrid.apiKey &&
          config.sendgrid.apiKey.length > 0 &&
          config.sendgrid.apiKey !== 'your-sendgrid-api-key' &&
          !config.sendgrid.apiKey.includes('SG.')) {
        logger.warn('SendGrid API key appears to be invalid. Falling back to SMTP.');
        this.initializeSMTP();
        ...
        // Fall back to SMTP for development (Mailhog or similar)
        this.initializeSMTP();
      }
    } catch (error: any) {
    ...
    try {
      ...
      switch (this.provider) {
        ...
        case 'ses':
          ...
          logger.info(`Email sent successfully via AWS SES to ${options.to}`, {

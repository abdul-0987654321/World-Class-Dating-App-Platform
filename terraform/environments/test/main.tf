...
# test environment with balanced settings for testing and QA.
...
module "storage_account" {
  ...
  replication_type           = "GRS"  # Geo-redundant for test
  soft_delete_retention_days = 14     # Moderate retention
...
module "container_registry" {
  ...
  location            = module.resource_group.location
  environment         = var.environment
  sku                 = "Standard"  # Upgraded for test
  admin_enabled       = true

      ...
      >
        ...
        {streaks.login && (
          <View style={styles.streakCard}>
            <LinearGradient
              colors={[
                getStreakLevel(streaks.login.currentStreak).color,
                getStreakLevel(streaks.login.currentStreak).color + 'CC',
              ]}
            ...
            >
              <View style={styles.streakCardHeader}>
                ...
                <View style={styles.streakCardInfo}>
                  ...
                  <Text style={styles.streakCardLevel}>
                    {getStreakLevel(streaks.login.currentStreak).level}
                  </Text>
                </View>
              ...
              <View style={styles.streakNumbers}>
                <View style={styles.streakNumberItem}>
                  <Text style={styles.streakNumberValue}>{streaks.login.currentStreak}</Text>
                  <Text style={styles.streakNumberLabel}>Current</Text>
                </View>
                <View style={styles.streakNumberDivider} />
                <View style={styles.streakNumberItem}>
                  <Text style={styles.streakNumberValue}>{streaks.login.longestStreak}</Text>
                  <Text style={styles.streakNumberLabel}>Longest</Text>
                </View>
              </View>
              {!streaks.login.isProtected && (
                <TouchableOpacity
                  style={styles.protectButton}
                  onPress={() => handleProtectStreak('login')}
                  disabled={protecting}
                >
                  <Ionicons
                    name="shield"
                    size={16}
                    color={getStreakLevel(streaks.login.currentStreak).color}
                  />
                  <Text
                    style={[
                      styles.protectButtonText,
                      { color: getStreakLevel(streaks.login.currentStreak).color },
                    ]}
                  >
        ...
        <View style={styles.section}>
          ...
          <View style={styles.milestonesList}>
            {milestones.map((milestone, index) => (
              <View
                key={index}
                style={[styles.milestoneItem, milestone.isAchieved && styles.milestoneItemAchieved]}
              >
                <View
                  style={[
                ...
                >
                  <Ionicons
                    ...
                    size={24}
                    color={milestone.isAchieved ? '#FFF' : '#999'}
                  />
                </View>
                <View style={styles.milestoneInfo}>
                  <Text
                    style={[
                      ...
                      milestone.isAchieved && styles.milestoneNameAchieved,
                    ]}
                  >
                    {milestone.name}
                  </Text>
                ...
                <View style={styles.milestoneRewards}>
                  {milestone.reward.coins && (
                    <View style={styles.milestoneReward}>
                      ...
                      <Text style={styles.milestoneRewardText}>+{milestone.reward.coins}</Text>
                    </View>
                  )}

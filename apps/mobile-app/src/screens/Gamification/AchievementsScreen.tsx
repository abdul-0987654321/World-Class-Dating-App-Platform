...
const AchievementsScreen: React.FC = () => {
  ...
  const renderAchievement = ({ item }: { item: Achievement }) => {
    ...
    return (
      <TouchableOpacity
        style={[styles.achievementCard, item.isUnlocked && styles.achievementCardUnlocked]}
        onPress={() => setSelectedAchievement(item)}
      >
        <LinearGradient
          colors={item.isUnlocked ? [item.iconColor, item.backgroundColor] : ['#ECF0F1', '#BDC3C7']}
        ...
        >
          <View style={styles.achievementIconContainer}>
            <Ionicons
              ...
              size={36}
              color={item.isUnlocked ? '#FFF' : '#95A5A6'}
            />
            {item.isUnlocked && (
          ...
          <View style={styles.achievementContent}>
            <View style={styles.achievementHeader}>
              <Text
                style={[styles.achievementName, !item.isUnlocked && styles.textLocked]}
                numberOfLines={1}
              >
                {item.name}
              </Text>
            ...
            <Text
              style={[styles.achievementDescription, !item.isUnlocked && styles.textLocked]}
              numberOfLines={2}
            >
              {item.description}
            </Text>
            ...
            <View style={styles.achievementRewards}>
              {item.coinReward > 0 && (
                <View style={styles.rewardBadge}>
                  ...
                  <Text style={styles.rewardBadgeText}>{item.coinReward}</Text>
                </View>
              )}
              {item.xpReward > 0 && (
                <View style={styles.rewardBadge}>
                  <Ionicons name="flash" size={14} color="#4CAF50" />
                  <Text style={styles.rewardBadgeText}>{item.xpReward}</Text>
                </View>
              )}
            </View>
          </View>
              ...
              <>
                <LinearGradient
                  colors={
                    selectedAchievement.isUnlocked
                      ? [selectedAchievement.iconColor, selectedAchievement.backgroundColor]
                      : ['#ECF0F1', '#BDC3C7']
                  }
                ...
                >
                  <Ionicons
                    ...
                    size={56}
                    color={selectedAchievement.isUnlocked ? '#FFF' : '#95A5A6'}
                  />
                </LinearGradient>
                ...
                <View style={styles.modalRewards}>
                  {selectedAchievement.coinReward > 0 && (
                    <View style={styles.modalRewardItem}>
                      ...
                      <Text style={styles.modalRewardValue}>+{selectedAchievement.coinReward}</Text>
                    </View>
                  )}
                  {selectedAchievement.xpReward > 0 && (
                    <View style={styles.modalRewardItem}>
                      <Ionicons name="flash" size={24} color="#4CAF50" />
                      <Text style={styles.modalRewardValue}>+{selectedAchievement.xpReward}</Text>
                    </View>
                  )}
                </View>
              </>

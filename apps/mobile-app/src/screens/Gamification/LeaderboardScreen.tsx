  ...
  useEffect(() => {
    const loadLeaderboard = async () => {
      ...
      if (leaderboardType === 'streak')
        data.sort((a, b) => b.streak - a.streak).forEach((e, i) => (e.rank = i + 1));
      else if (leaderboardType === 'achievements')
        data.sort((a, b) => b.achievements - a.achievements).forEach((e, i) => (e.rank = i + 1));
      setLeaderboard(data);
    ...
    return (
      <View style={[styles.entryCard, item.isCurrentUser && styles.entryCardCurrentUser]}>
        <View style={styles.entryRank}>
          <Text style={styles.entryRankText}>{item.rank}</Text>
        </View>
        <View style={styles.entryAvatar}>
          <Text style={styles.entryAvatarInitial}>{item.username.charAt(0)}</Text>
        </View>
        <View style={styles.entryInfo}>
          <Text
            style={[styles.entryUsername, item.isCurrentUser && styles.entryUsernameCurrentUser]}
          >
        ...
        <View style={styles.entryScore}>
          ...
          <Text style={styles.entryScoreLabel}>
            {leaderboardType === 'xp' ? 'XP' : leaderboardType === 'streak' ? 'days' : 'badges'}
          </Text>
        </View>
      </View>
    );
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <FlatList
          ...
          <>
                    ...
                    <View key={entry.id} style={styles.podiumItem}>
                      <View style={[styles.podiumAvatar, index === 1 && styles.podiumAvatarFirst]}>
                        ...
                        <View
                          style={[
                            styles.rankBadge,
                            { backgroundColor: getRankColor(entry.rank)[0] },
                          ]}
                        >
                      ...
                      <LinearGradient
                        ...
                        style={[
                          styles.podiumBase,
                          { height: index === 1 ? 100 : index === 0 ? 80 : 60 },
                        ]}
                      />
                    </View>
                  ))}
            ...
            <View style={styles.typeSelectorContainer}>
              {(['xp', 'streak', 'achievements'] as LeaderboardType[]).map((type) => (
                ...
                >
                  <Ionicons
                    name={type === 'xp' ? 'flash' : type === 'streak' ? 'flame' : 'trophy'}
                    size={18}
                    color={leaderboardType === type ? '#FFF' : '#666'}
                  />
                  <Text
                    style={[
                  ...
                  >
                    {type === 'xp' ? 'XP' : type === 'streak' ? 'Streak' : 'Badges'}
                  </Text>
                </TouchableOpacity>
              ))}
            </View>
        ...
        refreshControl={
          <RefreshControl refreshing={refreshing} onRefresh={refresh} colors={['#FF6B6B']} />
        }
      />
    </SafeAreaView>
  );

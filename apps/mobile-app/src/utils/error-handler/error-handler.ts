 ...
 * This triggers appropriate UX behavior (navigation, alerts, etc.)
 */
export function handleErrorByStatus(error: ProcessedError): void {
  ...
  switch (status) {
    case 401:
      // Navigate to login, clear auth state
    ...
    case 402:
      ...
      if (globalConfig.onBillingRequired) {
        ...
        globalConfig.showAlert?.(userMessage, 'warning');
      }
      break;
    ...
    case 403:
      ...
      if (errorCode === PermissionErrorCode.PREMIUM_REQUIRED ||
        ...
        globalConfig.showAlert?.(userMessage, 'error');
      }
      break;

    case 404:
      // Show not found screen or alert

...
export function useLocationAnomalyCheck() {
  return useAsyncOperation<
    any,
    [string, { latitude: number; longitude: number; ip_address?: string }]
  >((userId, location) => fraudDetectionService.checkLocationAnomaly(userId, location));
}

export function useDeviceCheck() {
  return useAsyncOperation<any, [string, FraudCheckRequest['device']]>((userId, device) =>
...
export function useScamDetection() {
  return useAsyncOperation<
    { is_scam: boolean; confidence: number; indicators: string[] },
    [string]
...
export function useIcebreakers() {
  ...
  >((bio, targetInterests, senderInterests) =>
...
export function usePhotoModeration() {
  return useAsyncOperation<any, [string]>((photoUrl) =>
    photoAnalysisService.moderatePhoto(photoUrl)
  );
}

export function usePhotoRanking() {
  return useAsyncOperation<any, [string, string[]]>((userId, photoUrls) =>
...
export function useRecommendations() {
  return useAsyncOperation<RecommendationResult, [any]>((request) =>
...
export function useSimilarProfiles() {
  return useAsyncOperation<any[], [string, string, number?]>((userId, likedUserId, limit) =>

...
export function useDeviceCheck() {
  return useAsyncOperation(
    (userId: string, device: FraudCheckRequest['device']) =>
...
export function useProfileFraudAnalysis() {
  return useAsyncOperation(
    (userId: string) => fraudDetectionService.analyzeProfile(userId)
  );
}
...
export function useSentimentAnalysis() {
  return useAsyncOperation<SentimentAnalysisResult, [string, string?]>(
    (text, context) => nlpService.analyzeSentiment(text, context as 'message' | 'bio' | 'prompt' | 'general')
  );
}

export function useToxicityDetection() {
  return useAsyncOperation<ToxicityAnalysisResult, [string, boolean?]>(
    (text, strictMode) => nlpService.analyzeToxicity(text, strictMode)
  );
}
...
export function useScamDetection() {
  return useAsyncOperation<ScamDetectionResult, [string]>(
...
export function useIcebreakers() {
  return useAsyncOperation(
    (bio: string, targetInterests: string[], senderInterests: string[]) =>
...
export function useBioAnalysis() {
  return useAsyncOperation(
    (bio: string) => nlpService.analyzeBioQuality(bio)
  );
}
...
export function usePhotoModeration() {
  return useAsyncOperation(
    (photoUrl: string) => photoAnalysisService.moderatePhoto(photoUrl)
  );
}

export function usePhotoRanking() {
  return useAsyncOperation(
    (userId: string, photoUrls: string[]) => photoAnalysisService.rankPhotos(userId, photoUrls)
  );
}

export function useFakePhotoDetection() {
  return useAsyncOperation(
    (photoUrl: string) => photoAnalysisService.detectFakePhoto(photoUrl)
  );
}

export function usePhotoSuggestions() {
  return useAsyncOperation(
    (userId: string, photoUrls: string[]) => photoAnalysisService.getPhotoSuggestions(userId, photoUrls)
  );
}

export function useSelfieVerification() {
  return useAsyncOperation(
    (userId: string, selfieFile: File, referencePhotoUrl: string) =>
...
export function useRecommendations() {
  return useAsyncOperation<RecommendationResult, [Parameters<typeof recommendationService.getRecommendations>[0]]>(
...
export function useSimilarProfiles() {
  return useAsyncOperation(
    (userId: string, likedUserId: string, limit?: number) =>
...
export function useLikedYou() {
  return useAsyncOperation(
    (userId: string, limit?: number, blurred?: boolean) =>
...
export function useRecommendationStats() {
  return useAsyncOperation(
    (userId: string) => recommendationService.getRecommendationStats(userId)
  );
}
  ...
  const verifyProfile = useCallback(async (
    ...
    try {
      ...
      const data = {
        ...
        fakeDetection: results[1] as Awaited<ReturnType<typeof photoAnalysisService.detectFakePhoto>>,
...
export function useProfileQualityAssessment() {
  ...
  const assessProfile = useCallback(async (
    userId: string,
    bio: string,
    photoUrls: string[]
  ) => {

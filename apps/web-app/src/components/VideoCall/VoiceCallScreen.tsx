    ...
    try {
      ...
      if ('setSinkId' in HTMLMediaElement.prototype) {
        ...
        const targetDevice = isSpeakerOn
          ...
          : availableDevices.find(
              (d) => d.label.toLowerCase().includes('speaker') || d.deviceId === 'default'
            );
    ...
    return (
      <div className={`flex items-end gap-0.5 ${color}`}>
        {[1, 2, 3, 4].map((i) => (
          <div
            key={i}
            className={`w-1 rounded-sm transition-all ${i <= bars ? 'bg-current' : 'bg-white/30'}`}
            style={{ height: `${i * 4}px` }}
          />
        ))}
      </div>
      ...
      <div className="relative h-full flex flex-col items-center justify-between py-12 px-6">
          ...
          <div className="relative mb-6">
            ...
            <motion.div
              ...
              animate={{
                ...
                opacity:
                  status === 'connected' && !isHoldActive ? 0.6 + remoteAudioLevel * 0.4 : 0.3,
              }}
        ...
        <div className="w-full max-w-md">
          ...
          <div className="flex items-center justify-center gap-6 mt-4">
            ...
            <span className="text-white/60 text-sm w-16 text-center">Speaker</span>
            <span className="text-white/60 text-sm w-20 text-center">End</span>
            {onSwitchToVideo && (
              <span className="text-white/60 text-sm w-16 text-center">Video</span>
            )}
          </div>
        </div>
      </div>
            ...
            {availableDevices.map((device) => (
              ...
              >
                <div className="w-8 h-8 rounded-full bg-white/10 flex items-center justify-center">
                  {device.label.toLowerCase().includes('bluetooth') ? (
                    <svg viewBox="0 0 24 24" className="w-4 h-4 text-blue-400">
                      <path
                        fill="currentColor"
                        d="M14.24 12.01l2.32 2.32c.28-.72.44-1.51.44-2.33s-.16-1.59-.44-2.32l-2.32 2.33zm1.88-4.4l.9-.9c-.58-.93-1.3-1.78-2.14-2.5l-1.42 1.42c.59.49 1.1 1.04 1.52 1.65l1.14 .33z"
                      />
                    </svg>
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-white text-sm truncate">{device.label || 'Unknown Device'}</p>
                </div>
              </button>
            ))}
          </motion.div>

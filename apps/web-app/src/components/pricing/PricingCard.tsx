...
const PricingCard: React.FC<PricingCardProps> = memo(
  ({
    ...
    const monthlyEquivalent =
      interval === 'yearly' && plan.price > 0 ? (parseFloat(displayPrice) / 12).toFixed(2) : null;
      ...
      >
        ...
        <div className={`bg-gradient-to-br ${styles.gradient} p-fm-5`}>
          ...
          <div className="flex items-center gap-3 mb-4">
            <div className={`${styles.iconBg} p-2 rounded-lg text-white`}>{icon}</div>
          ...
          {monthlyEquivalent && (
            <p className="text-white/60 text-xs mt-1">${monthlyEquivalent}/mo billed annually</p>
          )}
        </div>
          ...
          <ul className="space-y-3 mb-6">
            {plan.features.slice(0, 6).map((feature, idx) => (
              <li key={idx} className="flex items-start gap-2">
                ...
                <span className="text-sm text-fm-text-secondary leading-tight">{feature}</span>
              </li>
            ))}
          </ul>

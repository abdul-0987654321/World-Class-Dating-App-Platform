...
const VerificationTab: React.FC<{ status: VerificationStatus | null }> = ({ status }) => {
  if (!status) return <p>Unable to load verification status.</p>;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="text-center mb-8">
        <div className="inline-flex items-center justify-center w-24 h-24 rounded-full bg-gradient-to-r from-pink-500 to-purple-600 text-white text-2xl font-bold mb-3">
          {status.verificationScore}%
        </div>
        <h3 className="text-lg font-semibold text-gray-800">Verification Score</h3>
      ...
      {status.socialMediaVerified.length > 0 && (
        <div className="mt-6">
          <h4 className="font-semibold text-gray-800 mb-3">Linked Social Accounts</h4>
...
const SecurityTab: React.FC<{
  ...
  if (!settings) return <p>Unable to load security settings.</p>;
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Login Alerts */}
      <div className="border-b pb-6">
        <h3 className="text-lg font-semibold text-gray-800 mb-4">Login Alerts</h3>
      ...
      {/* Trusted Devices */}
      <div>
        <h3 className="text-lg font-semibold text-gray-800 mb-4">Trusted Devices</h3>
        {settings.trusted_devices.length > 0 ? (
          ...
          <p className="text-gray-500">No trusted devices configured.</p>
        )}
      </div>
    </div>
  );
...
const PrivacyTab: React.FC<{
  ...
  if (!settings) return <p>Unable to load privacy settings.</p>;
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Advanced Privacy */}
      <div>
        <h3 className="text-lg font-semibold text-gray-800 mb-4">Advanced Privacy</h3>
        <div className="space-y-2">
          <Toggle
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-600">People who will be notified in case of emergency</p>
        </div>
        <button
          onClick={() => setShowForm(true)}
          className="bg-pink-500 hover:bg-pink-600 text-white px-4 py-2 rounded-lg transition flex items-center gap-2"
        >
          <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <form onSubmit={handleSubmit} className="bg-gray-50 rounded-lg p-4 space-y-4">
          ...
          <div className="flex items-center gap-4">
            ...
            <label className="flex items-center gap-2">
              <input
                ...
                className="rounded text-pink-500 focus:ring-pink-500"
              />
              <span className="text-sm text-gray-700">Notify on SOS</span>
            </label>
          </div>
          <div className="flex gap-2">
            <button
              type="submit"
              disabled={saving}
              className="bg-pink-500 hover:bg-pink-600 text-white px-4 py-2 rounded-lg transition disabled:opacity-50"
            >
      ...
      {contacts.length > 0 ? (
        <div className="space-y-3">
          {contacts.map((contact) => (
            <div
              key={contact.id}
              className={`flex items-center justify-between p-4 rounded-lg border ${
                contact.is_primary ? 'border-pink-300 bg-pink-50' : 'border-gray-200 bg-white'
              }`}
            >
              <div>
                <div className="flex items-center gap-2">
                  ...
                  {contact.is_primary && (
                    <span className="bg-pink-500 text-white text-xs px-2 py-0.5 rounded-full">Primary</span>
                  )}
                </div>
        ...
        <div className="text-center py-8">
          ...
          <p className="text-sm text-gray-400">Add someone you trust to be notified in emergencies.</p>
        </div>
      )}
    </div>
  );
...
const ResourcesTab: React.FC<{
  ...
  return (
    <div className="space-y-8">
      {/* Safety Tips */}
      <div>
        <h3 className="text-lg font-semibold text-gray-800 mb-4">Safety Tips for Dating</h3>
            ...
            {tipsByCategory.before.map((tip, idx) => (
              <li key={idx} className="flex items-start gap-2">
                <span className={`w-2 h-2 rounded-full mt-2 ${
                  tip.priority === 'high' ? 'bg-red-500' : tip.priority === 'medium' ? 'bg-yellow-500' : 'bg-gray-400'
                }`} />

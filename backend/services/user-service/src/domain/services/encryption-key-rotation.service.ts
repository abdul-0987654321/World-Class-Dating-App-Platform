...
export class EncryptionKeyRotationService {
  ...
  private getOldKey(): EncryptionKey {
    ...
    if (!masterKey || !salt) {
      throw new Error(
        'Old encryption key environment variables not set (TOTP_ENCRYPTION_MASTER_KEY_OLD, TOTP_ENCRYPTION_KEY_SALT_OLD)'
      );
    }
  ...
  async startKeyRotation(): Promise<string> {
    ...
    try {
      ...
      if (newKey.version <= oldKey.version) {
        throw new Error(
          `New key version (${newKey.version}) must be greater than old version (${oldKey.version})`
        );
      }

      // Create rotation record
      const [rotationRecord] = await trx('encryption_key_rotations')
        .insert({
      ...
      for (const record of totpRecords) {
        try {
          ...
          const reencryptedSecret = this.encryptWithKey(
            decryptedSecret,
            newKey.key,
            newKey.version
          );
          ...
          // Update record
          await trx('user_two_factor_auth').where({ id: record.id }).update({
  ...
  async getRotationStatus(rotationId: string): Promise<any> {
    const rotation = await db('encryption_key_rotations').where({ id: rotationId }).first();
  ...
  async getRotationHistory(): Promise<any[]> {
    const rotations = await db('encryption_key_rotations').orderBy('started_at', 'desc').limit(50);

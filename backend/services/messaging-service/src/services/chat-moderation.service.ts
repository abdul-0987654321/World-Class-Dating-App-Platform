...
class ChatModerationService {
  ...
  private checkLinks(content: string): ModerationFlag | null {
    ...
    if (links.length > SPAM_CONFIG.maxLinksPerMessage) {
      return {
        type: 'link',
        severity: 'medium',
    ...
    const suspiciousDomains = ['bit.ly', 'tinyurl', 'shorturl', 'goo.gl', 't.co'];
    const hasSuspiciousLink = links.some((link: string) =>

...
import { validateBody, validateQuery, SendGiftDto, GiftHistoryQueryDto } from '../../dto';
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (!isParticipant) {
        return res
          .status(403)
          .json({ success: false, error: 'Not a participant in this conversation' });
      }
...
router.get(
  '/history',
  authenticate,
  validateQuery(GiftHistoryQueryDto),
  async (req: Request, res: Response) => {
    try {
      const userId = (req as any).user?.id;
      ...
      const transactions = await virtualGiftsService.getUserGiftHistory(
        userId,
        type,
        limit,
        offset

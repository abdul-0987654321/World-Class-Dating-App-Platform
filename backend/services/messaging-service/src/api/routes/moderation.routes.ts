...
router.post('/report', authenticate, async (req: Request, res: Response) => {
  try {
    ...
    const isParticipant = conversation && (conversation.participant1Id === reporterId || conversation.participant2Id === reporterId);
    if (!conversation || !isParticipant) {
      return res.status(403).json({ success: false, error: 'Not authorized to report this conversation' });
    }

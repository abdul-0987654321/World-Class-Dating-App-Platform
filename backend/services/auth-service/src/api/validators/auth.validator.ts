...
const consentSchema = Joi.object({
  ...
  terms_accepted: Joi.boolean().valid(true).messages({
    'any.only': 'You must accept the Terms of Service to register',
  }),
  privacy: Joi.boolean().valid(true).messages({
    'any.only': 'You must accept the Privacy Policy to register',
  }),
  privacy_accepted: Joi.boolean().valid(true).messages({
    'any.only': 'You must accept the Privacy Policy to register',
  }),
  marketing: Joi.boolean().optional().default(false),
  marketing_emails: Joi.boolean().optional().default(false),
  data_sharing: Joi.boolean().optional().default(false),
})
  .or('terms', 'terms_accepted')
  .or('privacy', 'privacy_accepted')
  .required()
  .messages({
...
export const registerSchema = Joi.object({
  ...
  date_of_birth: Joi.date().max('now').custom(ageValidation).messages({
    'date.max': 'Date of birth cannot be in the future',
    'date.minAge': 'You must be at least 18 years old to register',
  }),
  dateOfBirth: Joi.date().max('now').custom(ageValidation).messages({
    'date.max': 'Date of birth cannot be in the future',
    'date.minAge': 'You must be at least 18 years old to register',
  }),
  gender: Joi.string()
...
export const verify2FASchema = Joi.object({
  ...
  tempSecret: Joi.string().optional(),
});

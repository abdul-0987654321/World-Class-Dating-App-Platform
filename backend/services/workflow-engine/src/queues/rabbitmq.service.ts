...
export class RabbitMQService implements OnModuleInit, OnModuleDestroy {
  ...
  constructor(
    ...
    this.queuePrefix = this.configService.get<string>('rabbitmq.queuePrefix') || 'flamoral_';
    this.exchanges = this.configService.get<Record<string, string>>('rabbitmq.exchanges') || {};
  }
      ...
      this.channel.consume(queueName, async (msg) => {
        if (msg) {
          try {
            ...
            this.logger.error(`Error processing message from ${queueName}: ${error.message}`);
  ...
  private async handleMatchCreated(msg: any): Promise<void> {
    await this.workflowExecutor.triggerWorkflows(TriggerType.MATCH_CREATED, msg, msg.userId);
  }

  private async handleMessageSent(msg: any): Promise<void> {
    await this.workflowExecutor.triggerWorkflows(TriggerType.MESSAGE_SENT, msg, msg.senderId);
  }

  private async handleLikeReceived(msg: any): Promise<void> {
    await this.workflowExecutor.triggerWorkflows(TriggerType.LIKE_RECEIVED, msg, msg.receiverId);
  }

  private async handleSuperLike(msg: any): Promise<void> {
    await this.workflowExecutor.triggerWorkflows(TriggerType.SUPER_LIKE, msg, msg.receiverId);
  }
  ...
  private async handleCoinPurchase(msg: any): Promise<void> {
    await this.workflowExecutor.triggerWorkflows(TriggerType.COIN_PURCHASE, msg, msg.userId);
  }

  private async handleFirstLogin(msg: any): Promise<void> {
    await this.workflowExecutor.triggerWorkflows(TriggerType.FIRST_LOGIN, msg, msg.userId);
  }

  private async handleProfileCompleted(msg: any): Promise<void> {
    await this.workflowExecutor.triggerWorkflows(TriggerType.PROFILE_COMPLETED, msg, msg.userId);
  }

  private async handleAbandonedOnboarding(msg: any): Promise<void> {
    await this.workflowExecutor.triggerWorkflows(TriggerType.ABANDONED_ONBOARDING, msg, msg.userId);
  }

  private async handleUserInactive7d(msg: any): Promise<void> {
    await this.workflowExecutor.triggerWorkflows(TriggerType.USER_INACTIVE_7D, msg, msg.userId);
  }

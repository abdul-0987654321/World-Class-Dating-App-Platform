...
output "enabled_standards" {
  ...
  value = compact([
    var.enable_cis_standard ? aws_securityhub_standards_subscription.cis[0].standards_arn : null,
    var.enable_pci_dss_standard ? aws_securityhub_standards_subscription.pci_dss[0].standards_arn : null,
    var.enable_nist_standard ? aws_securityhub_standards_subscription.nist[0].standards_arn : null,
    var.enable_aws_foundational_standard ? aws_securityhub_standards_subscription.aws_foundational[0].standards_arn : null,
  ])
}
...
output "product_subscriptions" {
  description = "Map of enabled product subscription ARNs"
  value       = { for k, v in aws_securityhub_product_subscription.main : k => v.arn }
}
...
output "finding_aggregator_arn" {
  ...
  value       = var.aggregation_region != null ? aws_securityhub_finding_aggregator.main[0].arn : null
}
...
output "pci_dss_standard_arn" {
  ...
  value       = var.enable_pci_dss_standard ? aws_securityhub_standards_subscription.pci_dss[0].standards_arn : null
}

output "nist_standard_arn" {
  description = "ARN of the NIST standard subscription"
  value       = var.enable_nist_standard ? aws_securityhub_standards_subscription.nist[0].standards_arn : null
}
...
output "aws_foundational_standard_arn" {
  ...
  value       = var.enable_aws_foundational_standard ? aws_securityhub_standards_subscription.aws_foundational[0].standards_arn : null
}

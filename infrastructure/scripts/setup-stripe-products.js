 ...
 *   3. Run: node infrastructure/scripts/setup-stripe-products.js
 *
...
async function findExistingProduct(stripe, tierKey) {
  try {
...
async function findExistingPrice(stripe, productId, amountInCents) {
  try {
...
async function createProductAndPrice(stripe, tier, dryRun) {
...
async function main() {
  ...
  if (!stripeSecretKey) {
    ...
    console.error('Or on Windows: set STRIPE_SECRET_KEY=sk_live_...');
    process.exit(1);
  }
